...
UBYTE Dynamic_Refresh_Example(IT8951_Dev_Info Dev_Info, UDOUBLE Init_Target_Memory_Addr){
    ...
    while(1)
    {
        ...
        for(Dynamic_Area_Width = 64, Dynamic_Area_Height = 64; (Dynamic_Area_Width < Panel_Width - 32) && (Dynamic_Area_Height < Panel_Height - 24); Dynamic_Area_Width += 32, Dynamic_Area_Height += 24)
        {
...
void run_all_examples(IT8951_Dev_Info Dev_Info, UDOUBLE Init_Target_Memory_Addr) {
    uint16_t Panel_Width  = Dev_Info.Panel_W;
    uint16_t Panel_Height = Dev_Info.Panel_H;

    //Show 16 grayscale
    Display_ColorPalette_Example(Panel_Width/16, Panel_Height, Init_Target_Memory_Addr);
	EPD_IT8951_Clear_Refresh(Dev_Info, Init_Target_Memory_Addr, GC16_Mode);
